...
\chapter{Data structure A}

\section{Structure description}

Data structure A keeps inserted elements in a list, where each element has been assigned a tag from [0, \textit{u}].

Let \textit{n} be the current number of elements in the data structure.

Let \textit{u} be the tag universe size. The tag universe size is a power of 2.

The \textit{density} of a \textit{tag range} is the fraction of occupied tags in that tag range.\\

\textbf{Analogy to binary tree}\\

The bits of the tags implicitly form a binary tree which is the \textit{trie} of the bit strings. The trie has depth log(\textit{u}). Each root-to-leaf path in the trie corresponds to the bits of the tag of the element stored in the reached leaf node. This trie is called the \textit{virtual tree}.\\
When new elements are inserted into the list, they are assigned new tags, which translates to inserting new leaf nodes into the virtual tree.\\
...
\section{Insert operation \textit{Insert(X, Y)}}
...
+ Indirection => constant-time amortized solution to order-maintenance problem.

\chapter{Data structure B}

\chapter{Data structure C}
