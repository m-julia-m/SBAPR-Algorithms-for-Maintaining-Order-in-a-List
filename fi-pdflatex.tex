...
Solution A keeps inserted elements in a linked list, where each element has been assigned an integer tag from range [0, \textit{u}]. The tag values are strictly increasing over the list.
...
The \textit{density} of a \textit{tag range} is the fraction of occupied (already assigned) tags from the tag range.\\
...
\section{Delete operation \textit{Delete(X)}}

When deleting an element form the linked list, there are no excess steps on top of the standard delete operation in a linked list.

\section{Order operation \textit{Order(X, Y)}}
Since tag values increase as we traverse the linked list, we simply compare the tags of elements X and Y. If X has a lower tag than Y, then X precedes Y.
